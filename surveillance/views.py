...
# surveillance/views.py  (add near the top)


# ── WebSocket token for Node.js auth ─────────────────────────────────────────

def ws_token(request):
    if not request.user.is_authenticated:
        return JsonResponse({'error': 'unauthenticated'}, status=401)
    token = pyjwt.encode(
        {'user_id': request.user.id, 'exp': int(time.time()) + 60},
        settings.SECRET_KEY,
        algorithm='HS256'
    )
...
# ── Internal endpoint Node.js calls to mark a notification read ───────────────
...
_redis = redis.Redis.from_url(os.environ.get('REDIS_URL', 'redis://127.0.0.1:6379'))

def _push_to_node(room: str, payload: dict):
    """Publish a message that the Node server fans out to the right room."""
    _redis.publish('surveillance_events', json.dumps({
        'room':    room,
        'payload': payload,
    }))
...
def _push_notification(recipient, notification_type, title, message, assignment=None, event=None):
    ...
    # Push to Node instead of Django Channels
    _push_to_node(f'user_{recipient.id}', {
        'type':              'NOTIFICATION',
...
def assign_target(request, target_pk):
    ...
    _push_notification(
        ...
        message=(
            _("You have been assigned to monitor '%(name)s %(last_name)s'.\n"
              "Assigned by: %(assigned_by)s\n"
              "Note: %(note)s") % {
                'name': target.name,
...
def pass_back_target(request, assignment_pk):
    ...
    if uploader:
        _push_notification(
            ...
            message=(
                _("Operator %(username)s has passed back the target "
                  "'%(name)s %(last_name)s'.\n"
                  "Assignment ID: #%(assignment_pk)s") % {
                    'username': request.user.username,
                    'name': assignment.target.name,
                    'last_name': assignment.target.last_name,
...
def verify_detection(request, event_pk):
    ...
    label = _("approved") if action == 'approve' else _("rejected")
