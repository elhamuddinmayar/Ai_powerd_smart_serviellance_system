...
/* ── WebSocket via Socket.IO → Node.js server ─────────────── */
(function initWS() {
  if (!document.querySelector('.b-nav')) return;  // only on authenticated pages

  const ioScript   = document.createElement('script');
  ioScript.src     = 'http://localhost:4000/socket.io/socket.io.js';
  ioScript.onerror = () => console.warn('[BF] Could not load Socket.IO — WS disabled');
  ioScript.onload  = () => {
    if (typeof io === 'undefined') {
      console.warn('[BF] io still not defined after script load');
      return;
    }

    // 1. Get a short-lived JWT from Django
    fetch('/ws-token/')
      .then(r => r.json())
      .then(({ token }) => {
        if (!token) return;

        // 2. Connect to Node.js WebSocket server
        ...
        socket.on('connect', () => {
          console.log('[BF] Socket.IO connected');
        });

        socket.on('connect_error', err => {
          console.warn('[BF] Socket.IO error:', err.message);
        });
        ...
        socket.on('STAT_UPDATE', msg => {
          document.querySelectorAll(`[data-stat-camera="${msg.camera_id}"]`)
            .forEach(el => { el.textContent = msg.count; });
        });

        socket.on('CAMERA_STATUS', msg => {
          document.querySelectorAll(`[data-cam-status="${msg.camera_id}"]`)
            .forEach(el => {
        ...
        window.BFSocket = {
          send: (data) => {
            if (data.type === 'MARK_READ') {
              socket.emit('MARK_READ', { notification_id: data.notification_id });
        ...
        window.markRead = (id) => socket.emit('MARK_READ', { notification_id: id });
      })
      .catch(err => console.warn('[BF] ws-token fetch failed:', err));
  };
